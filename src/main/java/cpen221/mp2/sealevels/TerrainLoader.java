...
public class TerrainLoader {
    ...
    private static boolean isKeyFor(File keyFile, String source) throws IOException {
        ...
        return new BufferedReader(new FileReader(keyFile)).readLine().equals(source);
    }
    ...
    private static Terrain loadWebTerrain(String source, DownloadNotifier downloadNotifier) throws IOException {        
        ...
        if (!keyFile.exists() || !dataFile.exists() || !isKeyFor(keyFile, source)) {
             ...
             * https://stackoverflow.com/questions/30405695/java-nio-filechannels-track-progress
             */
            ...
            ReadableConsumerByteChannel rcbc = new ReadableConsumerByteChannel(rbc, connection.getContentLength(), downloadNotifier);
